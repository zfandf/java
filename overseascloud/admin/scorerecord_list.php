...
$Table=new TableClass("scorerecord","sid");
AjaxHead();//禁止页面缓存

if(empty($action)){
	InitGP(array("type","orderby","orderway","keywords")); //初始化变量全局返回
	if(!empty($type))$wherestr[]="type='{$type}'";
	if(!empty($raction))$wherestr[]="action='{$raction}'";
	if(!empty($keywords))$wherestr[]=" CONCAT(sid,' ',uname,' ',remark) like '%$keywords%' ";
	...
	//包含后台模板文件
	include("tpl/scorerecord_list.htm");
