...
if(empty($action)){
	...
	if(!empty($keywords))$countwherestr[]=" CONCAT(gid,' ',uname,' ',goodsname,' ',goodsseller) like '%$keywords%' ";
	if(!empty($keywords))$wherestr[]=" CONCAT(O.gid,' ',O.uname,' ',O.goodsname,' ',O.goodsseller) like '%$keywords%' ";
	...
	if(empty($wheresql)){$wheresql='1';}
	    ...
	    $sql="select O.gid as gid,O.uname as uname,O.goodsurl as goodsurl, O.sendprice as sendprice, O.goodsseller as goodsseller, O.goodsprice as goodsprice, O.state as ostate,O.goodsremark as goodsremark, O.goodsname as goodsname, O.goodssize as goodssize, O.goodscolor as goodscolor, O.goodsnum as goodsnum, O.addtime as addtime, O.uptime as uptime,O.goodslianxiren as goodslianxiren,O.goodstel as goodstel,O.goodsaddress as goodsaddress,O.postcode as postcode,U.utype as utype from {$tablepre}songli as O left join {$tablepre}users as U on O.uid=U.uid where {$wheresql} order by {$orderstr} limit $offset,$pagesize";
