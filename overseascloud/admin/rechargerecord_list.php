...
InitGP(array("page","action","state","date","ids","did","delids")); //初始化变量全局返回
...
if(empty($action)){
	...
	//包含后台模板文件
	include("tpl/rechargerecord_list.htm");
	
	
	
}elseif ($action=="clear" && !empty($date)){
	//清空几天前的未附款订单
	$oldtime=time()- $date*24*3600;
	$wherestr="state=1 and addtime < $oldtime";
	DB::query("delete from {$Table->table} where $wherestr");
	showmsg("清理成功！",PHP_SELF);
