...
InitGP(array("action","common","tid","gid","page","gtypeid","taoid","txtcommentss","share_like","tao_id","type","user")); //初始化变量全局返回
$goodsobj=new TableClass('goods','gid');
...
if($action==''){
	...
	//获取该用户的更多分享-3
	$share_uid = $value['uid'];
	$sharearray3=$goodsobj->getdata(3,"Audit=1 and uid='".$share_uid."'",'listorder desc,gid desc','gid,uid,uname,gtypeid,goodsurl,goodsname,goodsprice,goodsseller,goodsimg,addtime');	
	//分享_评论
	$wherestr[]="gid=$gid";
	$wherestr[]="state=1";
	if(!empty($wherestr)) $wheresql = implode(' AND ', $wherestr);	//条件汇总
