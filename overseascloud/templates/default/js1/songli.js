...
function toShoppingCart() {
	...
	if ($.trim($("#goodslianxiren").val()).length == 0) {
		alert("请填写联系人");
		$("#goodslianxiren").attr("class", "red");	
		$("#goodslianxiren").focus();
		return false;
	}

	if ($.trim($("#goodstel").val()).length == 0) {
		alert("请填写电话号码");
		$("#goodstel").attr("class", "red");
		$("#goodstel").focus();		
		return false;
	}
	if ($.trim($("#goodsaddress").val()).length == 0) {
		alert("请填写详细地址");
		$("#goodsaddress").attr("class", "red");	
		$("#goodsaddress").focus();		
		return false;
	}
	if ($.trim($("#postcode").val()).length == 0) {
		alert("请填写邮政编码");
		$("#postcode").attr("class", "red");	
		$("#postcode").focus();		
		return false;
	}	
    ...
    $.ajax({
      ...
      //    contentType: "application/json;utf-8",
        ...
        success: function(d) { if (d=='OK') step3(); else if (d=='LOGIN') window.open('/user.php?action=loginandreg','_blank'); else alert("您提交的商品信息有误。"); }
    });
}
