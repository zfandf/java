...
if($num > $perpage) {
   ...
   $multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="'.$mpurl.'page=1" class="first"'.$ajaxtarget.'>1 ...</a>' : '').
    ...
    ($curpage > 1 && !$simple ? '<a href="'.$mpurl.'page='.($curpage - 1).'" '.$ajaxtarget.'>'.lang('onpage').'</a>' : '');

   for($i = $from; $i <= $to; $i++) {

    $multipage .= $i == $curpage ? '<span class="page-cur">'.$i.'</span>' :
   ...
   $multipage .= ($curpage < $pages && !$simple ? '<a href="'.$mpurl.'page='.($curpage + 1).'" class="next"'.$ajaxtarget.'>'.lang('nextpage').'</a>' : '').
   ...
   $multipage = $multipage ? '<span class="page-skip">&nbsp;'.(!$simple ? ''.lang('Total').''.$num.''.lang('Article').'' : '').$multipage.'</span>' : '';
