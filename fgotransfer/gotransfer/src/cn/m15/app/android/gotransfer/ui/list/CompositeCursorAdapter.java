...
public abstract class CompositeCursorAdapter extends BaseAdapter {
     ...
     */
     public void clearPartitions() {
        for (Partition partition : mPartitions) {
            ...
            if (partition.cursor != null && !partition.cursor.isClosed()) {
            	partition.cursor.close();
            }
    ...
    /**
     * Given a list position, check if it the last section of partition
     */
    public boolean isLastSectionInPartition(int position) {
        int start = 0;
        for (int i = 0, n = mPartitions.size(); i < n; i++) {
            int end = start + mPartitions.get(i).count;
            if (position == end-1) {
